...
<div class="select-none content flex min-h-screen flex-col bg-[#ffffff] text-white">
	<section class="flex flex-wrap justify-center">
		...
		>
			...
			<section class="p-2 flex flex-row items-center gap-4">
				<a
					href="/"
					class="bg-[#5291ad] hover:bg-[#4994ce] px-6 py-3 rounded-xl text-white font-bold transition-all duration-300 hover:scale-105 poppins-semibold"
				>
					← Voltar ao Menu
				</a>
				<img
	...
	<div class="m-3 flex flex-1 flex-wrap content-center items-center justify-center gap-4">
		{#each cards as { icon, name, flipped, matchFound }, idx}
			...
			<Card mouseClick={() => clicked(card)} iconSrc={icon} {name} flipped={!matchFound && flipped} />
		{/each}
	</div>
